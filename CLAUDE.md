...
- **Compose = `postgres` + `migrator` (one-shot) + `api` (`:8080`, waits for migrator)**. The **Worker is NOT containerized** — it drives a *headed* browser (headless is WAF-blocked), and containers have no display, so it needs xvfb (or stealth) + is currently a one-shot CLI; containerize it alongside the Quartz scheduler. **Dockerfiles must `COPY Directory.Build.props` before restore** (D19) or the in-container build has no TargetFramework.
