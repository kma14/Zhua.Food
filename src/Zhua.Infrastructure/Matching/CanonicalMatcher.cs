...
/// <summary>
/// Offline canonical matcher (plan D9/D18). Two tiers:
/// <list type="bullet">
/// <item>Tier 1 — Foodstuffs (New World + PAK'nSAVE) share a <c>productId</c>, so each SKU becomes one canonical
/// (100% reliable, auto).</item>
/// <item>Tier 2 — Woolworths has no shared id/GTIN with Foodstuffs, so we match a Woolworths product to a
/// Foodstuffs-derived canonical by <c>brand + size</c> (hard filter) then name-token overlap: a single clearly-best
/// candidate auto-links; anything ambiguous/weaker goes to the <see cref="MatchCandidate"/> review queue.</item>
/// </list>
/// Idempotent: canonicals are upserted by <see cref="CanonicalProduct.MatchKey"/>, human decisions are honoured
/// and never re-proposed.
/// </summary>
public sealed class CanonicalMatcher(ZhuaDbContext db, TimeProvider clock) : ICanonicalMatcher
{
    ...
    public async Task<MatchRunResult> RunAsync(CancellationToken ct = default)
    {
        ...
        var canonByKey = await db.CanonicalProducts
            .Where(c => c.MatchKey != null)
            .ToDictionaryAsync(c => c.MatchKey!, ct);

        // --- Tier 1: Foodstuffs share productId → one canonical per SourceSku, link every branch's StoreProduct. ---
        ...
        foreach (var grp in foodstuffs.GroupBy(p => p.SourceSku))
        {
            ...
            foreach (var sp in grp)
                sp.CanonicalProduct = canon;
        }

        // --- Tier 2: index Foodstuffs canonicals by (brand, size) for Woolworths matching. ---
        var index = new Dictionary<(string, string), List<(CanonicalProduct canon, HashSet<string> tokens)>>();
        ...
        var rejected = decisions.Where(m => m.Status == MatchStatus.Rejected)
            .Select(m => (m.StoreProductId, m.CanonicalProductId)).ToHashSet();
        var known = decisions.Select(m => (m.StoreProductId, m.CanonicalProductId)).ToHashSet();
        ...
        foreach (var m in decisions.Where(m => m.Status == MatchStatus.Approved))
            if (byId.TryGetValue(m.StoreProductId, out var sp))
                sp.CanonicalProductId = m.CanonicalProductId;

        var autoLinkedWoolworths = 0;
        foreach (var w in products.Where(p => p.Store.Chain == Chain.Woolworths))
        {
            if (w.CanonicalProductId is not null) continue; // already linked (e.g. approved)
        ...
        var stale = await db.MatchCandidates
            .Where(m => m.Status == MatchStatus.Pending && m.StoreProduct.CanonicalProductId != null)
            .ToListAsync(ct);
        if (stale.Count > 0)
        {
        ...
        var totalCanon = await db.CanonicalProducts.CountAsync(ct);
        var linked = await db.StoreProducts.CountAsync(p => p.CanonicalProductId != null, ct);
        ...
        static string? FinestCategory(StoreProduct p)
        {
