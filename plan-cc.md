...
|---|---|---|---|---|
| D30.3 | 🧑‍⚖️ Category browse + count include unmatched listings (TD-5 paid down) | ✅ **Pending/held listings are now browsable.** Category browse required `p.ItemId != null && p.Item.CategoryId ∈ subtree`, so every unmatched listing (待审/悬空) was invisible to `/categories/{id}/products`, `/deals?category=` and the tree badge — only text search reached them (front-end's category report). Fixed via the "browse doesn't need an item" route (established during D30): a **hybrid** predicate (`ProductRepository.WhereInCategory`, shared by list+deals+count) — matched listing via `Item.CategoryId` (**unchanged, zero regression**) OR unmatched listing via its own `StoreCategory.CategoryId`; the tree count (`CountGroupsByCategoryAsync`) unions both so the badge matches the page. Client distinguishes single-store cards via the new `ProductGroup.comparable` (= >1 store) — Kevin chose to colour-code client-side rather than force a comparable-first sort. Chose the hybrid over the singleton-item approach TD-5 originally sketched (simpler, no new items, no auto-merge). Live: the eggs department now surfaces the held Henergy/Farmer Brown eggs; measured 100% of currently-unmatched listings (WW 836 + FC 422) have a mapped store-category so all become browsable. Residual (not TD-5): granularity bounded by source (FreshChoice department-only), and a listing under only *unmapped* store-categories still won't show (CategoryMapper coverage). 160 tests green. | browse (not search) is the primary discovery path, and hiding a quarter of the catalog behind search is a real gap; the hybrid keeps matched compare cards byte-for-byte unchanged while additively surfacing the rest, and reuses the already-proven fact that a listing reaches the shared tree via its store-category without needing an item | 2026-07-23 |
| D30.2 | 🧑‍⚖️ Two contained crawl/parse fixes (from a front-end category-browse bug report) | ✅ **Stale category pollution + FreshChoice egg size fixed.** (1) `CrawlOrchestrator` reset a product's *tags* each run but never its *category* links → `Product.Categories` accumulated across crawls; a colby cheese (`woolworths:281636`) carried 8 shelves (Barn Eggs + Haloumi + Block Cheese…) and `CategoryMapper`'s finest-first pick landed on "Barn Eggs". Now the category set is cleared on first sight per run, **complete crawls only** (a partial one didn't query every aisle — mirrors the D28 guard); existing pollution clears on the next complete crawl. (2) FreshChoice's size span is empty for pack-sold lines (eggs) → `RawSize` null → matcher's brand+size filter couldn't fire; `FreshChoiceParser` now recovers the pack count from the name ("12 Pack"→`"12pk"`), and `NormalizeSize` canonicalises multipack units (`12pack`≡`12pk`) so Woolworths' `"12pack"` matches. Live: Otaika Valley eggs now cross-store matched (FC↔Foodstuffs), auto-links 15→23. Verified each claim in code + live DB first; the third front-end ask (category browse depends on `Item.CategoryId` → 待审/悬空 invisible) is TD-5, scoped separately as the bigger query-layer change. | the colby-as-eggs bug came straight from the tag/category asymmetry (tags reset, categories didn't); FreshChoice eggs were structurally unmatchable without a size, and the pack-format drift between chains ("12pack" vs "12pk") was the last blocker — both are small, contained fixes worth doing before the larger TD-5 browse refactor | 2026-07-23 |
| D30.1 | 🧑‍⚖️ Categorise WW/FC-anchored items (department alias) + make the Tier-4 guard generic | ✅ **All 885 uncategorized items now categorised; guard fix shipped going-forward.** (1) `CategoryMapper` gains a small curated **department alias table** (`DepartmentAliases`, 7 entries) consulted after the exact (kind, slug) match fails, so Woolworths/FreshChoice departments that don't slug-match the Foodstuffs-seeded tree (WW "Meat & Poultry"/FC "Meat"/"Seafood" → "Meat, Poultry & Seafood", etc.) map in — categorised items 5792→**6677** (Uncategorized **885→0**). Chose option B (map ~7 nodes, self-maintaining) over option C (curate 885 items) — ~25× less work and it's the one curated user-facing surface (D25). Aisle/shelf-level mapping left for later (bounded by the Foodstuffs tree depth; promo/cross-cutting WW aisles like "3 for $20" deliberately not force-mapped). (2) Tier-4 guard widened from `foodstuffsBrands` to `foodstuffsBrands ∪ wwBrands` so a FreshChoice product sharing a Woolworths private label ("WW"/"Macro") is held, not minted a singleton. Since stable `MatchKey` froze the **89** pre-existing such singletons, a **reclaim** step (un-link + delete a `freshchoice:` singleton whose product now looks higher-tier, so it re-cascades) was added on Kevin's "先跑一遍" — live run reclaimed all 89 (items 6677→6588; re-cascaded to 53 review-queue + 36 held), `MatchRunResult.Reclaimed` reports it. | 885 WW/FC-anchored items (no Foodstuffs member) were Uncategorized purely because of a department-name mismatch, not missing data — a tiny alias table recovers all of them; the guard was non-generic (only Foodstuffs brands) so FC private-label products that belong with a Woolworths anchor leaked into singletons that split the compare | 2026-07-22 |
...
| D30 | 🧑‍⚖️ Anchor-priority cascade: Foodstuffs → Woolworths → FreshChoice | ✅ **Every active product gets an item; the anchor is chosen by source data quality.** `ItemMatcher` gains two tiers after the Foodstuffs-anchored Tier 1/2: **Tier 3** — a Woolworths product whose brand ∉ Foodstuffs-vocab becomes a `woolworths:{sku}` item, and FreshChoice attaches to it (brand inferred against the WW-anchor vocab, which carries private-label brands like `WW`/`Macro` the Foodstuffs vocab lacks); **Tier 4** — any FreshChoice listing that attached to nothing becomes a `freshchoice:{sku}` singleton. Correctness guard (quantified): a product only anchors a NEW item if its brand isn't a Foodstuffs brand — else it's a Tier-2 miss belonging to a Foodstuffs item, and anchoring would mint a duplicate that splits the compare (the guard leaves **861** such Woolworths listings unanchored — the one deliberate exception to the invariant). Reuses D29 brand-inference + the `(brand,size)` index + merge (higher tier wins the reverse edge). `MatchRunResult.AutoLinked` now excludes self-anchors (cross-links only). Live result: **+1,956** Woolworths-anchored items (**61** genuine WW+FC 2-store price groups — e.g. Alpine Cheese Colby $13.90 WW vs $14.90 FC, impossible to compare before), **+529** FreshChoice singletons; categorised products 4192→5792. Came out of the orphan-matching research ([docs/internals/orphan-matching.md](docs/internals/orphan-matching.md)); along the way verified in code that category-browse does NOT need items (a product reaches the shared tree via `StoreCategory.CategoryId`), so singletons are justified by the "every product has an ItemId" invariant + future merges, not browsability | cross-store compare is the platform's moat (§10) and was structurally impossible for products Foodstuffs doesn't stock while every item was Foodstuffs-anchored; anchoring by data quality (FS deterministic > WW has RawBrand > FC infers) keeps the higher-confidence source as the root and lets the weaker one attach | 2026-07-22 |
| D29 | 🧑‍⚖️ FreshChoice matcher coverage: brand inference + AutoLinked count fix | ✅ **FreshChoice listings now enter matching.** `ItemMatcher`'s Tier 2 loop widened from Woolworths-only to Woolworths ∪ FreshChoice; since FreshChoice publishes no `RawBrand` (D26), a brand is inferred by checking the listing name's leading 1–3 words (extending past a trailing joining "&", fixed same day after a "Beak & Sons" → "Beak &" truncation was spotted) against the vocabulary of brands Tier 1 already knows from Foodstuffs — self-bootstrapping, no maintained list, and a wrong guess is free (it just fails the `(brand,size)` index like any other unmatchable listing). Result on the live catalog: 306/1,241 auto-linked, 202 queued for review. Also fixed `MatchRunResult.AutoLinked`: it was `CountLinkedProductsAsync` — a DB-wide, all-time, all-store cumulative count mislabelled as "this run" — now a before/after diff scoped to the run's active-store products. Trigger: a front-end matching-coverage report, independently verified against the live DB (every number checked out except one unreproducible "81 missing size" figure) — findings: FreshChoice had zero matcher coverage, Woolworths' 2,452 zero-candidate listings are ~83% genuinely unmatchable (loose-sold/private-label/brand-not-stocked) and ~17% a real brand+size-format miss, and unmatched listings are invisible to category-filtered browsing (deferred as TD-5, [tech-debt.md](docs/internals/tech-debt.md) — bigger scope: singleton items + auto-merge-on-match) | FreshChoice's ~50% brand-derivable rate makes the dictionary approach worth it; a naive "always take the first word" heuristic was rejected because it can't guarantee a wrong guess is harmless (a coincidental token match against a real brand could propose a wrong candidate) | 2026-07-21 |
| D28 | 🧑‍⚖️ Crawl completeness + missing-product reconciliation + /deals freshness | ✅ **A crawl knows whether it covered the whole store, and products that vanish from complete crawls are retired.** (1) `IStoreCrawler.FetchAsync` → `ScrapeResult` (products + **gap list**); all 4 crawlers retry failed pages (3s/8s) and report surviving failures as gaps; a gapped run = `CrawlRunStatus.Partial` (summary in `ErrorMessage`), never `Succeeded`. (2) **Foodstuffs Algolia truncation**: `totalHits` pins at exactly **1000** (`paginationLimitedTo`) and pages >20 silently vanish (hit fridge-deli-eggs at all 3 PAK stores + NW Browns Bay); a capped scope splits by the response's `category1NI`/`category2NI` facets and crawls the sub-scopes. (3) After a **complete** run only, the orchestrator calls `Product.RecordMissing` on unseen listings: 1st miss starts the streak (`MissingSince`/`ConsecutiveMissingRuns`), **2nd consecutive miss** → `IsAvailable=false` + promo cleared, **no synthetic snapshot** (history = real observations only); reappearing resets in `ApplyObservation`. (4) Shopper queries exclude unavailable listings; `/deals` also requires `LastSeenAt` within **48h** (`DealQueries.FreshnessWindow`, via `TimeProvider`). Migration `AddProductAvailability` (backfill `IsAvailable=true`). Trigger: front-end bug report — Highland Park's Round Green Bean kept a 2026-07-13 special ($4.99/was $22.99) in `/deals` for a week after the branch delisted it (archive-verified: the 07-19 crawl was complete and the SKU genuinely absent) | a delisted product froze at its last-seen state forever — nothing retired unseen listings, and `/deals` trusted `IsOnSpecial` unconditionally; reconciliation needs the complete/partial distinction or one lost page mass-retires half a store; threshold 2 tolerates a single source hiccup; the freshness window covers a dead worker (reconciliation can't run then) | 2026-07-20 |
